...
#     # 4. Relative positioning for other karts
#     for k in karts:
#         if k is ego:
#             continue
...
#         # Horizontal Logic
#         h_pos = "left" if ox < ego_x else "right"
#         if h_pos == "left": num_left += 1
#         else: num_right += 1

#         # Specific questions (Strictly one-word answers)
#         qa_pairs.append({
#             "question": f"Is {k['kart_name']} in front of or behind the ego car?",
#             "answer": v_pos
#         })
...
def extract_kart_objects(
    ...
    """
    Extracts karts including those with 0-size bounding boxes to match ground truth.
    """
    with open(info_path) as f:
        info = json.load(f)
    ...
    # Handle kart naming
    if isinstance(info.get("karts"), list) and info["karts"]:
        id_to_name = {i: name for i, name in enumerate(info["karts"])}
    elif isinstance(info.get("kart_id_to_name"), dict):
        id_to_name = {int(k): str(v) for k, v in info["kart_id_to_name"].items()}

    # Scale factors based on the 600x400 coordinate system in the JSON
    scale_x = img_width / ORIGINAL_WIDTH
    scale_y = img_height / ORIGINAL_HEIGHT
    ...
    for det in frame_detections:
        ...
        if int(class_id) != 1: # Only process Karts
            continue

        # Scale coordinates to the model's input size (150x100)
        ...
        # Filter: Skip ONLY if the center is completely outside the image
    ...
    # Identify Ego: Prefer track_id 0, otherwise closest to screen center
    ego_idx = next((i for i, k in enumerate(karts) if k["instance_id"] == 0), None)
    if ego_idx is None:
        img_cx, img_cy = img_width / 2, img_height / 2
        ego_idx = min(range(len(karts)), 
...
def generate_qa_pairs(info_path: str, view_index: int, img_width: int = 150, img_height: int = 100) -> list:
    """
    Generates binary spatial logic QA pairs with concise answers.
    """
    karts = extract_kart_objects(info_path, view_index, img_width=img_width, img_height=img_height)
    if not karts: return []
    ...
    n_front, n_back, n_left, n_right = 0, 0, 0, 0

    for k in karts:
        if k is ego: continue
        kx, ky = k["center"]

        # Binary Vertical Logic: y decreases as we go 'up' into the distance
        v_pos = "front" if ky < ego_y else "back"
        if v_pos == "front": n_front += 1 
        else: n_back += 1

        # Binary Horizontal Logic
        h_pos = "left" if kx < ego_x else "right"
        if h_pos == "left": n_left += 1
        else: n_right += 1

        qa_pairs.append({"question": f"Is {k['kart_name']} in front of or behind the ego car?", "answer": v_pos})
        qa_pairs.append({"question": f"Is {k['kart_name']} to the left or right of the ego car?", "answer": h_pos})
        qa_pairs.append({"question": f"Where is {k['kart_name']} relative to the ego car?", "answer": f"{v_pos} and {h_pos}"})

    qa_pairs.append({"question": "How many karts are in front of the ego car?", "answer": str(n_front)})
    qa_pairs.append({"question": "How many karts are behind the ego car?", "answer": str(n_back)})
    qa_pairs.append({"question": "How many karts are to the left of the ego car?", "answer": str(n_left)})
    qa_pairs.append({"question": "How many karts are to the right of the ego car?", "answer": str(n_right)})

    return qa_pairs

def build_qa_dataset(split: str = "train", out_name: str = "generated", data_dir: str = "../data"):
    data_dir = Path(__file__).parent / data_dir
    split_dir = data_dir / split
    info_files = sorted(split_dir.glob("*_info.json"))
    qa_pairs_all = []
    ...
    for info_file in info_files:
        ...
        for view_idx in range(10):
            img_candidates = list(split_dir.glob(f"{base}_{view_idx:02d}_im.jpg"))
            if not img_candidates: continue
            
            image_rel = f"{split}/{img_candidates[0].name}"
            qa_list = generate_qa_pairs(str(info_file), view_idx)
            for qa in qa_list:
                qa_pairs_all.append({"image_file": image_rel, "question": qa["question"], "answer": qa["answer"]})
...
def main():
    fire.Fire({"build": build_qa_dataset})

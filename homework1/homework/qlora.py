...
class QLoRALinear(Linear4Bit):
    def __init__(
        ...
        # The assignment hint mentions "cast inputs to self.linear_dtype"
        # Linear4Bit doesn't define it, so we define it here.
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        #raise NotImplementedError()
        # Base path (quantized, frozen) computed in linear_dtype
        x_lin = x.to(self.linear_dtype)
        with torch.no_grad():
            base = super().forward(x_lin)  # returns float32 when x_lin is float32

        # LoRA path (trainable, float32)
